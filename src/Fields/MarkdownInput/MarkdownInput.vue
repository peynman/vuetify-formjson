<template>
    <div :class="`vf-input d-flex flex-column flex-grow-1 ${field.class ? field.class : ''}`">
        <v-toolbar v-show="!field.renderOnly" flat dense>
        ...
        <v-card v-show="mode === 'editor'" style="position: relative;" class="fill-hight mx-2">
            <div :style="`position: absolute; width: 100%; min-height: 200px; left: 5px;${field.height ? 'height: ' + field.height : ''}`" ref="editorCode">
            </div>
        </v-card>
    </div>
</template>
